// Python binding for dl-from-zero-3-concurrent (pybind11)
// 对应 TF 三层 API 的 Python 层; 图是静态的, sess.run 返回 fetch 的张量列表
...
PYBIND11_MODULE(lfpy, m) {
    m.doc() = "dl-from-zero-3-concurrent python binding";
    ...
    // 图节点: 纯静态, 只有 name/type (值在 RunState / Session, 不在节点上)
    ...
    py::class_<Graph>(m, "Graph")
        ...
        .def("sgd_step", [](Graph& g, Node* var, float lr) { return g.sgd_step(var, lr); },
             py::keep_alive<0, 1>(), py::return_value_policy::reference)
    ...
    py::class_<Session>(m, "Session")
        ...
        .def("run", [](Session& s, Graph& g, const py::list& targets,
                       const py::dict& feeds, py::object loss_node) -> py::list {
            ...
            auto results = s.run(g, t, feed_map, loss);
            py::list out;
