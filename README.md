...
| [tensorflow/dl-from-zero](2026/tensorflow/dl-from-zero/) | C++ / Python | 从 0 到 1 复刻 TensorFlow 概念 — 7 个演进版本（v0-linear 手写 autodiff → v1-prune 图裁剪+binding → v2-ps-worker 分布式 → v3-concurrent 图/状态分离 → v4-grad 独立梯度子图 → v5-opt 图优化 CSE/常量折叠 → v6-quant int8 量化推理），总文档见 umbrella README |
